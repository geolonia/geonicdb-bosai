...
/** 通知シェードに残る 1 件（`Notification` の最小面）。 */
export type NotificationLike = { close: () => void };

/** `ServiceWorkerRegistration.getNotifications()` の最小面。 */
export type NotificationSource = {
  getNotifications?: () => Promise<NotificationLike[]>;
};
...
export async function closeAllNotifications(
  ...
  if (!source || typeof source.getNotifications !== "function") return;
  try {
