...
**中間サーバーは不要。** Web Push は静的アセット配信基盤（例: GitHub Pages、本番の S3 + CloudFront）と
GeonicDB の 2 者で完結する。
...
- バナー／お知らせ更新で通知が来ないのは、**新規または再作成済みの購読**では仕様（#48。警戒レベル変更のみ）。旧仕様の既存購読が残っている場合は届きうる（SW は `default` 文言）
