...
- ライセンス: public リポジトリ（自治体・ベンダーによるフォーク利用を想定）
...
コンテンツ更新は `geonic` CLI や Claude Desktop（MCP）から GeonicDB の NGSI-LD API を直接叩く運用です。CMS の構築・運用・脆弱性対応が丸ごと不要になります。権限は XACML ポリシーでサーバー側に寄せ、公開ページには API キーを埋め込みません。
