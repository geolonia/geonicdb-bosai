...
| `PUSH_OPT_IN_BANNER_REVEAL_IDLE_MS` | `30_000` | 無操作のまま帯を出すまでの待ち（ミリ秒） |

帯はヘッダーより上の文書フローに入るため、ロード直後に差し込むとページ全体が下へずれて CLS になります。
初回の `click` / `keyup` があれば即出し、無操作なら `PUSH_OPT_IN_BANNER_REVEAL_IDLE_MS` だけ待ってから出します
（A2HS 帯と同じ方針）。
